...
function ReturnBadge({ value, label, color }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span
        className="inline-block w-6 h-0.5 rounded-full shrink-0"
    ...
    <div className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2.5 shadow-xl text-xs">
      ...
      {port && (
        <p className="text-blue-400 font-semibold">
          Portfolio: <span className="text-white">{fmt(port.value)}</span>
        </p>
      )}
      {spy && (
        <p className="text-slate-400 font-semibold">
          S&P 500: <span className="text-white">{fmt(spy.value)}</span>
        </p>
      )}
    </div>
  )
...
export default function PerformanceChart({ sessionId, compact = false }) {
  ...
  const lastPort = data?.portfolio[data.portfolio.length - 1]
  const lastSpy  = data?.spy[data.spy.length - 1]
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            {!compact && (
              <p className="text-slate-500 text-xs">Normalized to 100 at period start</p>
            )}
          </div>
        </div>
